  ...
  return (
    <div className="glass-card-hover p-5">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="p-2 rounded-lg bg-surface-800" style={{ color: accent }}>
      ...
      <div
        ...
        className="text-4xl font-medium text-heat-100 leading-none"
      >
        {value}
        {unit && <span className="text-xl text-surface-500 ml-1">{unit}</span>}
      </div>

      {sub && <div className="text-xs text-surface-500 mt-2">{sub}</div>}
    </div>
  );
}
...
function HeatBadge({ risk }: { risk: RiskLevel }) {
  ...
  return (
    <div
      className={`relative flex flex-col items-center justify-center p-8 rounded-2xl border ${cfg.bg} ${cfg.border} overflow-hidden`}
    >
      {pulsing && (
      ...
      <div className="relative z-10 flex flex-col items-center gap-3">
        ...
        <div>
          <div className="text-center text-xs text-surface-500 uppercase tracking-wider mb-1">
            Heat Risk Level
          </div>
          <div
            className={`font-display font-bold text-4xl text-center ${cfg.text}`}
          >
            {risk}
          </div>
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <div
            className="flex items-center gap-2 bg-surface-800/80 border border-surface-700/60
            rounded-xl px-3.5 py-2"
          >
            <Clock className="w-3.5 h-3.5 text-surface-500" />
            <span className="font-mono text-heat-200 text-[13px]">
              {timeStr}
            </span>
          </div>

          <div
            className="flex items-center gap-2 bg-heat-600/15 border border-heat-600/25
            rounded-xl px-3.5 py-2"
          >
        ...
        <div className="lg:col-span-2 glass-card p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="flex items-center gap-1.5 mono-label">
                <span className="w-3 h-px bg-ember-400 inline-block opacity-60" />{' '}
                Feels
              </span>
            </div>
          </div>
                  ...
                  <div
                    className={`flex items-center gap-4 cursor-pointer rounded-lg px-2 py-1 -mx-2
                      transition-colors duration-150 ${
                        isActive ? 'bg-heat-600/10' : 'hover:bg-surface-800/40'
                      }`}
                    onClick={() => {
                    ...
                    <div className="w-24 shrink-0">
                      <div
                        className={`text-[13px] font-medium transition-colors ${
                          isActive ? 'text-heat-300' : 'text-heat-200'
                        }`}
                      >
                        {zone.name}
                      </div>
