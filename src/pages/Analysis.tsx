...
import { zones, historicalData, heatIndexTable, riskConfig } from '../lib/mockData';
...
const zoneBarData = zones.map((z) => ({ name: z.name, temp: z.temperature, color: z.color }));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in space-y-8">
      ...
      <div>
        ...
        <p className="text-surface-400 text-sm">Comparative zone data, historical trends, and heat index reference</p>
      </div>
      ...
      <div className="glass-card p-6">
        <div className="mb-6">
          ...
          <p className="text-xs text-surface-500">Current surface temperatures across all 6 monitored zones</p>
        </div>
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={zoneBarData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
            ...
            <YAxis tick={{ fill: '#6b7280', fontSize: 11 }} tickLine={false} axisLine={false} domain={[28, 45]} unit="°" />
              ...
              <div key={zone.id} className="glass-card-hover p-5">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="font-display font-semibold text-heat-100">{zone.name}</span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div className="text-center bg-surface-800 rounded-lg p-2">
                    <div className="font-bold text-base" style={{ color: zone.color }}>{zone.temperature}°</div>
                    <div className="text-[10px] text-surface-500">Temp</div>
                  </div>
                  <div className="text-center bg-surface-800 rounded-lg p-2">
                    <div className="font-bold text-base text-stone-400">{zone.concreteRatio}%</div>
                    <div className="text-[10px] text-surface-500">Concrete</div>
                  </div>
                  <div className="text-center bg-surface-800 rounded-lg p-2">
                    <div className="font-bold text-base text-emerald-400">{zone.greenCover}%</div>
                    <div className="text-[10px] text-surface-500">Green</div>
                  </div>
                </div>
                <p className="text-xs text-surface-400 leading-relaxed line-clamp-4">{zone.description}</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="glass-card p-6">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-700">
                ...
                <th className="text-center text-xs text-surface-400 font-medium pb-3 px-2">30% RH</th>
                <th className="text-center text-xs text-surface-400 font-medium pb-3 px-2">50% RH</th>
                <th className="text-center text-xs text-surface-400 font-medium pb-3 px-2">70% RH</th>
                <th className="text-center text-xs text-surface-400 font-medium pb-3 px-2">90% RH</th>
              </tr>
            </thead>
            <tbody>
        ...
        <p className="text-xs text-surface-500 mt-3">
          Current conditions: {38}°C at {72}% RH — <span className="text-red-400 font-medium">Extreme Danger level</span>
        </p>
      </div>
    </div>
  );
